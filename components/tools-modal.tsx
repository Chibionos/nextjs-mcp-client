...
import { X, Wrench, Search, Server, FileText, MessageSquare, ChevronDown, ChevronRight } from 'lucide-react';
  ...
  const filterItems = (items: any[], type: 'tool' | 'resource' | 'prompt') => {
    return items.filter(({ serverName, [type]: item }) => {
      const matchesSearch = searchQuery === '' || 
        ...
        (item.uri && item.uri.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <DialogContent className="max-w-6xl w-[90vw] h-[85vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                {allTools.length} tools, {allResources.length} resources, {allPrompts.length} prompts from {connectedServers.length} servers
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
              ...
              <div className="px-6 py-4">
                {Object.keys(toolsByServer).length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {(Object.entries(toolsByServer) as [string, any[]][]).map(([serverName, tools]) => (
                      <div key={serverName}>
                                        ...
                                        <div className="break-words overflow-wrap-anywhere word-break">
                                          <ReactMarkdown
                                            ...
                                            components={{
                                              ...
                                              code: ({ children }) => (
                                                <code className="bg-muted px-1 py-0.5 rounded text-xs break-all whitespace-pre-wrap">{children}</code>
                                              ),
            ...
            <ScrollArea className="h-full">
              <div className="px-6 py-4">
                {Object.keys(resourcesByServer).length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {(Object.entries(resourcesByServer) as [string, any[]][]).map(([serverName, resources]) => (
                      <div key={serverName}>
                        <div className="flex items-center gap-2 mb-3">
                          <Server className="w-4 h-4 text-muted-foreground" />
                          <h3 className="font-semibold">{serverName}</h3>
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-sm font-medium">{resource.name || resource.uri}</span>
                                  </div>
                                  {resource.uri && (
            ...
            <ScrollArea className="h-full">
              <div className="px-6 py-4">
                {Object.keys(promptsByServer).length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {(Object.entries(promptsByServer) as [string, any[]][]).map(([serverName, prompts]) => (
                      <div key={serverName}>
                        <div className="flex items-center gap-2 mb-3">
                          <Server className="w-4 h-4 text-muted-foreground" />
                          <h3 className="font-semibold">{serverName}</h3>
                                ...
                                <div className="flex items-start justify-between">
                                  <div className="flex-1 min-w-0">
                                    ...
                                    {prompt.arguments && prompt.arguments.length > 0 && (
                                      <div className="mt-3">
                                        <button
                                          onClick={() => toggleSchema(promptId)}
                                          className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors"
                                        >
                                          {isExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                                          View Arguments ({prompt.arguments.length})
                                        </button>
                                        {isExpanded && (
                                          <div className="mt-2 space-y-2">
                                            {prompt.arguments.map((arg: any, argIndex: number) => (
                                              <div key={argIndex} className="bg-background p-2 rounded text-xs">
                                                <div className="font-mono font-medium">{arg.name}</div>
                                                {arg.description && (
        ...
        <div className="border-t px-6 py-3 bg-muted/30 flex-shrink-0">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>
              Showing {filteredTools.length + filteredResources.length + filteredPrompts.length} items
            </span>
            <Button
              onClick={onClose}
              variant="outline"
