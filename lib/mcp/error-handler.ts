...
export class ErrorHandler {
    ...
    if (
      ...
      if (error?.code === -32601) {
        return {
          ...
          isRecoverable: false,
          suggestedAction:
            "This is expected - server does not support this method",
        };
      }
  ...
  static logError(error: MCPError): void {
    if (ErrorHandler.isExpectedError(error)) {
      ...
      console.info(
        `[MCP] ${error.serverName || "Unknown"}: SSE connection idle, will reconnect if needed`,
      );
      ...
      console.error(
        `[MCP] ${error.serverName || "Unknown"}: ${error.message}`,
        error.originalError,
      );
    }
  }
