  ...
  for (const block of content) {
    if (block.type === 'tool_use') {
      ...
      try {
        ...
        console.log(`Tool ${toolUse.name} result:`, JSON.stringify(result).substring(0, 500) + '...');
...
export async function POST(request: NextRequest) {
  try {
    ...
    let conversationMessages: any[] = [...claudeMessages];
    let maxRounds = 5; // Prevent infinite loops
    let round = 0;

    while ((currentResponse.stop_reason === 'tool_use' || currentResponse.content.some(block => block.type === 'tool_use')) && round < maxRounds) {
      console.log(`Tool use detected (round ${round + 1}), executing tools...`);

      // Execute tool calls for this round
      const toolResults = await executeToolCalls(manager, currentResponse.content);

      // Collect all tool calls and results
      const toolCallsThisRound = currentResponse.content.filter(b => b.type === 'tool_use').map(b => ({
        id: (b as any).id,
        name: (b as any).name,
      ...
      // Get next response
      console.log(`Getting response after tool execution (round ${round + 1})...`);
      ...
      console.log(`Response after round ${round + 1}:`, {
        ...
        has_more_tools: currentResponse.content.some(b => b.type === 'tool_use'),
      });
    ...
    if (allToolCalls.length > 0) {
      return NextResponse.json({
        message: {
          role: 'assistant',
          content: textContent || 'I searched for information but couldn\'t generate a response. Please try rephrasing your question.',
        },
